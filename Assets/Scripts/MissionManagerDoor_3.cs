...
public class MissionManagerDoor_3 : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int i = 0; i < 4; i++) {
            Debug.Log("Elemento " + i + ": " + Loader.elementi[i]);
        }
    ...
    IEnumerator MissioneCoroutine() {
        ...
        if (Loader.difficulty == "Facile") {
            if (Loader.statoPlayerDoor3[0] == 0) { //se è la prima volta che arrivi alla door1
                Loader.statoPlayerDoor3[0] = 1;
                Loader.statoPlayerDoor3[1] = Player.Instance.GetHealth();
                Loader.statoPlayerDoor3[2] = Player.Instance.pozioni;
                if (PersistentUI.Instance.Elisir1.activeInHierarchy) {
                    Loader.statoPlayerDoor3[3] = 1;
                }
                else {
                    Loader.statoPlayerDoor3[3] = 0;
                }
                if (PersistentUI.Instance.Elisir2.activeInHierarchy) {
                    Loader.statoPlayerDoor3[4] = 1;
                }
                else {
                    Loader.statoPlayerDoor3[4] = 0;
                }
                if (PersistentUI.Instance.Elisir3.activeInHierarchy) {
                    Loader.statoPlayerDoor3[5] = 1;
                }
                else {
                    Loader.statoPlayerDoor3[5] = 0;
                }

                //Loader.fraseDoor3 = LLMManager.loaderAnswers[0];
            }
            else {
                Player.Instance.SetHealth(Loader.statoPlayerDoor3[1]);
                PersistentUI.Instance.healthbar.SetHealth(Loader.statoPlayerDoor3[1]);
                Player.Instance.pozioni = (int)Loader.statoPlayerDoor3[2];
                PersistentUI.Instance.potionButton.GetComponentInChildren<PotionButton>().text.text = Loader.statoPlayerDoor3[2].ToString();
                if (Loader.statoPlayerDoor3[3] == 1) {
                ...
                if (Loader.statoPlayerDoor3[4] == 1) {
                ...
                if (Loader.statoPlayerDoor3[5] == 1) {
                ...
                //LLMManager.loaderAnswers[0] = Loader.fraseDoor3;
            }
        }
